...
<body class="childrenBody">
	...
	<script type="text/javascript">
	    ...
	    layui.extend({
			...
			//渲染数据表格
			 tableIns=table.render({
			    ...
			    ,url:'${ctx}/menu/loadAllMenu.do' //数据接口
			...
			$("#doSearch").click(function(){
				...
				tableIns.reload({
					url:"${ctx}/menu/loadAllMenu.do?"+params
				})
			});
		   ...
		   table.on('tool(menuTable)', function(obj){
			  ...
			  if(layEvent === 'del'){ //删除
			      //先判断当前菜单有没有子节点
                 $.post("${ctx}/menu/checkMenuHasChildren.do?id="+data.id,function(obj) {
                    if (obj.code >= 0){
                        layer.msg("当前菜单存在子节点，请先删除子节点");
                    }else {
                        layer.confirm('真的删除【' + data.title + '】这个菜单吗',function(index) {
                            //向服务器发送删除指令
                            $.post("${ctx}/menu/deleteMenu.do",{id:data.id},function(res) {
                                layer.msg(res.msg);
                                //刷新数据表格
                                tableIns.reload();
                                //刷新左边的树
			...
			var menuTree = dtree.render({
		          ...
		          url: "${ctx}/menu/loadMenuManagerLeftTreeJson.do?spread=1",  // 使用url加载（可与data加载同时存在）
				  icon: "2",
				  accordion:true
				});
		...
		function reloadTable(id){
			tableIns.reload({
				url:"${ctx}/menu/loadAllMenu.do?id="+id
			})
		}
	</script>
</body>
